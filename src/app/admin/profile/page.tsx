    ...
    fetch("/api/admin/profile")
      ...
      .then(res => {
        if (res?.profile) {
          setData(res.profile);
          try {
            setHeadlines(JSON.parse(res.profile.hero_headline || "[]"));
          } catch {
            setHeadlines([]);
          }
        }
    ...
    <AdminShell title="پروفایل" description="اطلاعات شخصی و هیرو صفحه اصلی">
      <div className="max-w-4xl space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-background border border-border rounded-xl focus:border-foreground/20 focus:outline-none"
                placeholder="آماده همکاری"
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">آواتار (مسیر)</label>
              <input
                type="text"
                dir="ltr"
                value={data.avatar}
        ...
        <div className="bg-card border border-border rounded-2xl p-6">
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium mb-2">خطوط عنوان (هر خط جداگانه)</label>
              ...
              <div key={i} className="flex gap-2 mb-2">
                <input
                  ...
                  onChange={e => {
                    ...
                    setHeadlines(newLines);
                  }}
                  className="flex-1 px-4 py-3 bg-background border border-border rounded-xl focus:border-foreground/20 focus:outline-none"
                />
                <button
                  onClick={() => setHeadlines(headlines.filter((_, j) => j !== i))}
            ...
            >
              <Plus className="w-4 h-4" /> افزودن خط
            </button>
          </div>
